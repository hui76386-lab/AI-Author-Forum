...
class EnglishAdminResponseMiddleware:
    """Guarantee zero Han characters in English admin and public HTML."""
    ...
    def __call__(self, request):
        response = self.get_response(request)
        english_surface = request.path_info.startswith(
            self.admin_prefixes + self.public_prefixes
        )
        if not english_surface:
            return response
        if not (translation.get_language() or "").lower().startswith("en"):
            return response
        if response.get("Content-Type", "").split(";", 1)[0].lower() != "text/html":
            return response
